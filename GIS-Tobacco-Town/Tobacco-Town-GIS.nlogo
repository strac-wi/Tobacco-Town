...
to add-outlets
  ...
  ask outlets with [BusinessTy = "Public House" or BusinessTy = "Nightclub"] [die] ;; Pubs and Clubs don't sell tobacco anymore
...
to add-smokers
  ...
  ;;Smoking Rate cumulative proportions - From SHS 2021 Smoking module
...
;; Wage cumulative proportions - Data from National Income Distributions 2018
  ...
  set wage-proportions-list [	0.01	0.05	0.10	0.17	0.23	0.30	0.36	0.48	0.57	0.65	0.71	0.76	0.81	0.87	0.92	0.95	0.98 1] ;; this is new
  set wage-list [100 150 200 250 300 250 400 500 600 700 800 900 1000 1200 1400 1600 2000 2500]
  ...
  ask smokers[
    ...
    (ifelse
        ...
        transport_type = "bus" [set speed 2.1]  ;; Walking to bus stop
        transport_type = "train" [set speed 2.1] ;; Walking to station
        )
...
to set-fuel-price
  ask smokers [
    (ifelse
      transport_type = "car"
      [set fuel-price 5.92] ;;Fuel price for 2018 (£/Gallon)
      [set fuel-price 0 ]
      )
  ]
end
...
to purchase
  ...
  set total-cost-eq-per-pack lput cost_equation_per_pack total-cost-eq-per-pack
end
...
to calc-price
 ...
 let min-value 9999
  ...
  let sum-function sum (map [ x -> c_discount ^ floor ((20 * (x - 1) + c_inventory) / c_smoking_rate) ] qq-list)
    ...
    let current-value (  ((( difference / c_speed + 1 / 12) * c_hourly_wage * 1 + difference * c_fuel-price / efficiency + quantity * c_price ) / sum-function ))
      ;; Cost Calculation
    ...
    if current-value < min-value [
      ...
      set current-q quantity
      set cost-at-r   (( difference / c_speed + 1 / 12) * c_hourly_wage * 1 + difference * c_fuel-price / efficiency + current-q * c_price ) / current-q
        ]
...
;;;;;;;;;;;;;;;;;;;;;;;;Updates all purchase-related variables
 ...
 set retailer-type [BusinessTy] of best_outlet
 set distance-for-purchase [difference] of best_outlet
 set packs-purchased [current-q] of best_outlet

 if packs-purchased mod 10 = 0 [
   set pack-price ( (pack-price * 7.63 ) / 10 )
 ]
...
to report-end-state
  ...
  set average_costs ( mean [ smoker-average-overall-costs ] of smokers)
...
<experiments>
  <experiment name="experiment" repetitions="3" runMetricsEveryStep="false">
    ...
    <steppedValueSet variable="retailer-density-cap" first="10" step="5" last="100"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
