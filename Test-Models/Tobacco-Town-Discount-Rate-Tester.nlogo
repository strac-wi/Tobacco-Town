...
;; Smoking rate initialization
   ...
   set discount (ifelse-value                                                ;; Discount is a term that devalues packs purchased in the future - dependent on smoking-rate
          smoking-rate <= 10 [ ( 0.61 + random-float 0.39 ) ]
          smoking-rate < 20 and smoking-rate > 10 [ (0.59 + random-float 0.41) ]
          smoking-rate = 20 [ (0.57 + random-float 0.43 ) ]
          smoking-rate > 20 [ (0.53 + random-float 0.47 ) ]
        )
...
to remove-earners
  if earners-to-keep = "High" [
  let agents-to-remove count smokers * 0.75  ; Calculate the number of agents to be removed (quarter of the total count)
  let sorted-agents sort-on [wage] smokers  ; Sort the agents in ascending order based on the variable

  repeat agents-to-remove [
    let agent-to-die first sorted-agents
    ask agent-to-die [ die ]  ; Ask the agent with the lowest variable value to die
    set sorted-agents but-first sorted-agents
  ]
  ]

    if earners-to-keep = "Low" [
  let agents-to-remove count smokers * 0.75  ; Calculate the number of agents to be removed (quarter of the total count)
  let sorted-agents sort-on [wage] smokers  ; Sort the agents in ascending order based on the variable

  repeat agents-to-remove [
    let agent-to-die last sorted-agents
    ask agent-to-die [ die ]  ; Ask the agent with the lowest variable value to die
    set sorted-agents but-last sorted-agents
  ]
  ]
...
<experiments>
  ...
  <experiment name="Baseline Quant" repetitions="8" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>run-fast</go>
    <metric>average-costs</metric>
    <metric>end-density</metric>
    ...
    <enumeratedValueSet variable="earners-to-keep">
      <value value="&quot;High&quot;"/>
      <value value="&quot;Low&quot;"/>
    </enumeratedValueSet>
  </experiment>
